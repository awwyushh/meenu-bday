...
export default class MainScene extends Phaser.Scene {
    ...
    onWin() {
        ...
        if (this.spawnTimer) this.spawnTimer.remove(false);
        if (this.coinSpawnTimer) this.coinSpawnTimer.remove(false);
        try { if (this.sound.get("bgmusic")) this.sound.get("bgmusic").stop(); } catch (e) { }

        const cx = this.cameras.main.centerX;
        const cy = this.cameras.main.centerY;
        this.add.rectangle(cx, cy, this.scale.width, this.scale.height, 0x000000, 0.6).setDepth(100);

        const videoKey = "congrats";

        const clickText = this.add.text(cx, cy, "Click to play", {
            font: '20px Arial',
            color: '#ffffff',
        ...
        clickText.once('pointerup', () => {
            window.open('assets/video/congrats.mp4', '_blank');
        });
    }
